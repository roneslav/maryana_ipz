import { Form, Formik, useField } from 'formik';
import * as Yup from 'yup';
import FormError from "../../components/formError/formError";
import { useNavigate } from 'react-router-dom';
import axios from 'axios'; // Додайте імпорт Axios

import './checkout.css';

const CustomTextInput = ({ label, ...props }) => {
    const [field, meta] = useField(props);
    return (
        <>
            <label htmlFor={props.id || props.name}>{label}</label>
            <input className="text-input" {...field} {...props} />
            <FormError touched={meta.touched} error={meta.error} />
        </>
    );
};

const CustomSelect = ({ label, ...props }) => {
    const [field, meta] = useField(props);
    return (
        <>
            <label htmlFor={props.id || props.name}>{label}</label>
            <select className="text-input" {...field} {...props}>
                <option value="">Select payment method</option>
                <option value="Credit Card">Credit Card</option>
                <option value="Paypal">Paypal</option>
            </select>
            <FormError touched={meta.touched} error={meta.error} />
        </>
    );
};

const goBack = () => {
    window.history.back();
};

const postOrder = async (order) => {
    try {
        const response = await axios.post("http://localhost:5001/api/orders", order);
        return response.data; // Повертаємо відповідь
    } catch (error) {
        console.error('Error posting order:', error);
        throw error; // Викидаємо помилку для обробки
    }
};

function Checkout() {
    const navigate = useNavigate();

    return (
        <section>
            <div className="checkout-box">
                <div className="checkout-box__formik">
                    <Formik
                        initialValues={{
                            customerName: '',
                            address: '',
                            date_of_birth: '',
                            phoneNumber: '',
                            paymentMethod: '',
                        }}
                        validationSchema={Yup.object({
                            customerName: Yup.string()
                                .min(3, 'Must be at least 3 characters')
                                .max(15, 'Must be 15 characters or less')
                                .required('Required'),
                            address: Yup.string()
                                .min(3, 'Must be at least 3 characters')
                                .max(15, 'Must be 15 characters or less')
                                .required('Required'),
                            date_of_birth: Yup.string()
                                .matches(/^(0[1-9]|[12][0-9]|3[01])\.(0[1-9]|1[0-2])\.\d{4}$/, 'Invalid date format')
                                .required('Required')
                                .test('is-adult', 'Must be at least 18 years old', function (value) {
                                    const currentDate = new Date();
                                    const selectedDate = value ? new Date(value.split('.').reverse().join('-')) : null; // Перетворюємо формат дати

                                    if (!selectedDate || isNaN(selectedDate.getTime())) {
                                        return true;
                                    }

                                    const ageDifference = currentDate.getFullYear() - selectedDate.getFullYear();
                                    const isAdult =
                                        ageDifference > 18 ||
                                        (ageDifference === 18 &&
                                            (currentDate.getMonth() > selectedDate.getMonth() ||
                                                (currentDate.getMonth() === selectedDate.getMonth() &&
                                                    currentDate.getDate() >= selectedDate.getDate())));

                                    return isAdult;
                                }),
                            phoneNumber: Yup.string()
                                .matches(/^\+\d{12}$/, 'Invalid phone number')
                                .required('Required'),
                            paymentMethod: Yup.string()
                                .oneOf(['Credit Card', 'Paypal'], 'Required')
                                .required('Required'),
                        })}
                        onSubmit={async (values, { setSubmitting, resetForm }) => {
                            try {
                                await postOrder(values); // Викликаємо postOrder
                                alert(JSON.stringify(values, null, 2));
                                resetForm();
                                navigate('/success');
                            } catch (error) {
                                alert('Failed to submit order. Please try again.'); // Обробка помилки
                            } finally {
                                setSubmitting(false);
                            }
                        }}
                    >
                        {props => (
                            <Form>
                                <div className='checkout-box__forms'>
                                    <h1 className='checkout-box__title'>Checkout</h1>
                                    <CustomTextInput label='Your name' name='customerName' type='text' placeholder='Frank' />
                                    <CustomTextInput label='Phone number' name='phoneNumber' type='text' placeholder='+38...' />
                                    <CustomTextInput label='Your address' name='address' type='text' placeholder='Franka, 13' />
                                    <CustomSelect label='Choose payment method' name='paymentMethod' />
                                    <CustomTextInput label='Date of birth' name='date_of_birth' type='text' placeholder='DD.MM.YYYY' />
                                </div>

                                <div className='btn-checkout'>
                                    <button className='btn-to-catalog' onClick={goBack}>
                                        Go back
                                    </button>
                                    <button type='submit'>
                                        {props.isSubmitting ? 'Loading...' : 'Submit'}
                                    </button>
                                </div>
                            </Form>
                        )}
                    </Formik>
                </div>
            </div>
        </section>
    );
}

export default Checkout;
